...
int MeasureStdUnorderedMap(Iterator first, Iterator last, uint32_t mask) {
  static std::unordered_map<K, V> std_hash_map;
  return MeasureHashMap(std_hash_map, first, last, mask);
}

int Measure() {
  volatile uint32_t mask = bloat::kDefaultMask;
  auto& pairs1 = GetPairs<std::pair<K1, V1>>();
  int rc = MeasureStdUnorderedMap<K1, V1>(pairs1.begin(), pairs1.end(), mask);

#ifdef PW_CONTAINERS_SIZE_REPORT_ALTERNATE_VALUE
  auto& pairs2 = GetPairs<std::pair<K1, V2>>();
  rc += MeasureStdUnorderedMap<K1, V2>(pairs2.begin(), pairs2.end(), mask);
#endif

#ifdef PW_CONTAINERS_SIZE_REPORT_ALTERNATE_KEY_AND_VALUE
  auto& pairs3 = GetPairs<std::pair<K2, V2>>();
  rc += MeasureStdUnorderedMap<K2, V2>(pairs3.begin(), pairs3.end(), mask);
#endif
